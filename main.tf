locals {
  ...
  substitutions = {
    ...
    service_account = "hpc-1-nodes@lab-gke-se.iam.gserviceaccount.com"
  }
  ...
  serviceaccount_configs   = { for filename in local.serviceaccount_files : replace(filename, ".yaml", "") => yamldecode(templatefile("${path.module}/config/iam/serviceaccount/${filename}", local.substitutions)) }
}
